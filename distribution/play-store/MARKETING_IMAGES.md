...
These are real captures of the running app, not mockups. They are generated by
`tool/screenshots/play_store_screenshots_test.dart`, which renders the actual
production widget tree with the bundled Manrope and IBM Plex Sans Arabic fonts
and a consistent sample dataset, then writes PNGs via `RenderRepaintBoundary`.

Regenerate them with:

```bash
flutter test tool/screenshots/play_store_screenshots_test.dart
cp build/play-screenshots/*.png distribution/play-store/screenshots/
```

Output is 1080 x 2160 (2:1), which satisfies the Play phone-screenshot aspect
limits. The generator asserts that no capture contains an error state, an
Arabic error state, or a loading spinner, so an unstubbed provider fails the
run instead of silently shipping a "Something went wrong" card.

Tracked copies live in `distribution/play-store/screenshots/`.

| Frame | Screenshot file | Screen |
| --- | --- | --- |
| 1 | `01-home-overview.png` | Dashboard: pending income, balances, accounts |
| 2 | `16-income-automation.png` | Salary estimate breakdown awaiting approval |
| 3 | `06-work-calendar.png` | Work month calendar and extra pay |
| 4 | `19-income-split.png` | Active automation with its 20/80 account split |
| 5 | `11-card-due-breakdown.png` | Credit facility due breakdown |
| 6 | `07-reports-overview.png` | Reports: cash-flow bars and net-flow line |
| 7 | `05-money-transactions.png` | Unified transaction history |
| 8 | `17-home-arabic-rtl.png` | Dashboard, Arabic RTL |
| 9 | `01-home-overview.png` | Feature graphic source |

Additional captures available for rotation into the listing:

| File | Screen |
| --- | --- |
| `02-home-due-breakdown.png` | Due breakdown sheet from the dashboard |
| `03-home-cards-cashflow.png` | Cards and cash-flow section |
| `04-money-accounts.png` | Accounts list |
| `08-reports-charts.png` | Expenses, allowances, and income by category |
| `09-network-contacts.png` | Network contacts |
| `10-network-transfer-accept.png` | Incoming transfer to accept |
| `12-pay-choose-what-to-pay.png` | Selectable payment allocation |
| `13-shared-installment.png` | Shared installment link |
| `14-salary-periods.png` | Salary periods history |
| `15-recurring-payments.png` | Recurring payments |
| `18-home-dark.png` | Dashboard, dark mode |
