-- The Home due-breakdown sheet gets the same authoritative item-level paid
-- state as the facility Due Breakdown: every statement item in the
-- `details.items` array now carries paid_minor / remaining_minor /
-- payment_status aggregated from credit_card_statement_item_allocations,
-- and installment entries expose payment_status (and, for BNPL rows, the
-- amount and paid figures they previously omitted). All existing keys are
-- preserved unchanged so installed clients keep working; the new keys are
-- additive only. Fully paid items stay listed — the sheet shows them
-- checked and struck through instead of hiding them.
