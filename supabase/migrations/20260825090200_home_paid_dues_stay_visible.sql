-- Fully paid dues stay visible on Home, exactly like the facility Due
-- Breakdown keeps settled components checked instead of hiding them. A card
-- statement or BNPL due that has been paid remains in the Home obligations
-- while its due date has not passed (obligation_status = 'paid',
-- remaining_minor = 0), so the dues tiles and the breakdown sheet can show
-- "Paid in full" with checked, struck-through components rather than the due
-- silently vanishing the moment it is settled. Once the due date passes, a
-- settled due drops out as before — it is history, not a current due.
