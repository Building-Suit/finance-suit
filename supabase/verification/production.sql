...
begin
  ...
  if to_regclass('app_finance.bnpl_purchase_obligations') is null then
    v_missing := array_append(
      v_missing, 'app_finance.bnpl_purchase_obligations');
  end if;
  if to_regclass('app_finance.bnpl_purchase_payment_allocations') is null then
    v_missing := array_append(
      v_missing, 'app_finance.bnpl_purchase_payment_allocations');
  end if;
  if to_regclass('app_finance.bnpl_purchase_obligation_statuses') is null then
    v_missing := array_append(
      v_missing, 'app_finance.bnpl_purchase_obligation_statuses');
  end if;
  if not exists (
    select 1 from pg_proc p
    join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'app_finance' and p.proname = 'bnpl_purchase_due_on'
  ) then
    v_missing := array_append(
      v_missing, 'app_finance.bnpl_purchase_due_on');
  end if;
