...
select is((select count(*)::int from app_finance.accounts), 1, 'onboarding created one account');
select is((select count(*)::int from app_finance.transaction_categories), 22, 'default categories seeded');
select ok((select onboarding_completed_at is not null from app_core.user_preferences), 'onboarding marked complete');
select ok((select is_default from app_finance.accounts limit 1), 'first account is default');
...
select throws_ok(
  $$insert into app_finance.financial_transactions (user_id, transaction_kind, occurred_on, amount_minor, source_account_id)
    values ('00000000-0000-0000-0000-00000000000c', 'expense', current_date, 0,
            (select id from app_finance.accounts limit 1))$$,
  '23514', null, 'zero amount rejected');

-- Direction rule: expense cannot have destination.
select throws_ok(
  $$insert into app_finance.financial_transactions (user_id, transaction_kind, occurred_on, amount_minor, source_account_id, destination_account_id)
    values ('00000000-0000-0000-0000-00000000000c', 'expense', current_date, 100,
            (select id from app_finance.accounts limit 1), (select id from app_finance.accounts limit 1))$$,
...
select is((select balance_minor from app_finance.account_balances where name = 'Current Balance'), 700000::bigint, 'source reduced');
select is((select balance_minor from app_finance.account_balances where name = 'Savings'), 500000::bigint, 'destination increased');
select is((select sum(balance_minor)::bigint from app_finance.account_balances), 1200000::bigint, 'total unchanged');
...
select lives_ok(
  $$select app_salary.record_salary_payment(
      (select id from app_salary.salary_periods where period_start = '2026-06-01'),
      1000000,
      (select id from app_finance.accounts where name = 'Current Balance'),
      '2026-07-25', null)$$,
...
select results_eq(
  $$select income_minor, expenses_minor, allowances_minor from app_reports.cash_flow_summary('2026-01-01', '2099-12-31')$$,
